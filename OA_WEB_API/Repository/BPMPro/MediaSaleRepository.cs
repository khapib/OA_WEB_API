    ...
    public class MediaSaleRepository
    {
        ...
        public MediaSaleViewModel PostMediaSaleSingle(MediaSaleQueryModel query)
        {
            ...
            if (mediaSaleViewModel.APPLICANT_INFO.DRAFT_FLAG == 0)
            {
                ...
                //保留原有資料
                strJson = jsonFunction.ObjectToJSON(mediaSaleViewModel);
                ...
                if (mediaSaleViewModel.ASSOCIATED_FORM_CONFIG != null && mediaSaleViewModel.ASSOCIATED_FORM_CONFIG.Count > 0) BpmSystemOrder.IS_ASSOCIATED_FORM = true;
                else BpmSystemOrder.IS_ASSOCIATED_FORM = false;
                //確認是否有正常到系統起單；清除失敗表單資料並重新送單值行
                if (commonRepository.PostBPMSystemOrder(BpmSystemOrder)) PutMediaSaleSingle(jsonFunction.JsonToObject<MediaSaleViewModel>(strJson));
