...

{
    ...
    public class MediaOrderChangeRepository
    {
        ...
        public MediaOrderChangeViewModel PostMediaOrderChangeSingle(MediaOrderChangeQueryModel query)
        {
            ...
            if (mediaOrderChangeViewModel.APPLICANT_INFO.DRAFT_FLAG == 0)
            {
                #region - 確認BPM表單是否正常起單到系統中 -

                //保留原有資料
                strJson = jsonFunction.ObjectToJSON(mediaOrderChangeViewModel);
                ...
                if (mediaOrderChangeViewModel.ASSOCIATED_FORM_CONFIG != null && mediaOrderChangeViewModel.ASSOCIATED_FORM_CONFIG.Count > 0) BpmSystemOrder.IS_ASSOCIATED_FORM = true;
                else BpmSystemOrder.IS_ASSOCIATED_FORM = false;
                //確認是否有正常到系統起單；清除失敗表單資料並重新送單值行
                if (commonRepository.PostBPMSystemOrder(BpmSystemOrder)) PutMediaOrderChangeSingle(jsonFunction.JsonToObject<MediaOrderChangeViewModel>(strJson));
                ...
                if (String.IsNullOrEmpty(mediaOrderChangeViewModel.MEDIA_ORDER_CHANGE_CONFIG.BPM_FORM_NO) || String.IsNullOrWhiteSpace(mediaOrderChangeViewModel.MEDIA_ORDER_CHANGE_CONFIG.BPM_FORM_NO))
                {
                    strSQL = "";
                    strSQL += "SELECT ";
                    strSQL += "     [BPMFormNo] AS [BPM_FORM_NO] ";
                    ...
                    if (dtBpmFormNo.Rows.Count > 0) mediaOrderChangeViewModel.MEDIA_ORDER_CHANGE_CONFIG.BPM_FORM_NO = dtBpmFormNo.Rows[0][0].ToString();
                }
