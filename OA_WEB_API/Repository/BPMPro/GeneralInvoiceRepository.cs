    ...
    public class GeneralInvoiceRepository
    {
        ...
        public GeneralInvoiceViewModel PostGeneralInvoiceSingle(GeneralInvoiceQueryModel query)
        {
            ...
            if (generalInvoiceViewModel.APPLICANT_INFO.DRAFT_FLAG == 0)
            {
                ...
                {
                    generalInvoiceViewModel = new GeneralInvoiceViewModel();
                    CommLib.Logger.Error("行政採購請款單(查詢)失敗，原因：系統無正常起單。");
                }
                else
                {
                    #region - 確認M表BPM表單單號 -
        ...
        public bool PutGeneralInvoiceSingle(GeneralInvoiceViewModel model)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    strSQL = "";
                    strSQL += "INSERT INTO [BPMPro].[dbo].[FM7T_GeneralInvoice_M]([RequisitionID],[DiagramID],[ApplicantDept],[ApplicantDeptName],[ApplicantID],[ApplicantName],[ApplicantPhone],[ApplicantDateTime],[FillerID],[FillerName],[Priority],[DraftFlag],[FlowActivated],[FlowName],[FormNo],[FM7Subject],[IsCFO]) ";
